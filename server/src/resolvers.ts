...
function createResolvers(db: Db): Resolvers<ResolverContext> {
  const resolvers: Resolvers<ResolverContext> = {
    ...
    Trend: {
      // __resolveType is a special method that helps the GraphQL server
      // determine which type a given object should be treated as when
      // resolving a union or interface field. In this case, we're using
      // it to distinguish between two types of trends: HashtagTrend and
      // TopicTrend. We can tell the difference between the two by checking
      // if the object has a hashtag field. If it does, it's a HashtagTrend.
      // Otherwise, it's a TopicTrend. This method is required when resolving
      // a union or interface field. If it's not provided, the server will
      // throw an error when trying to resolve the field.
      // Trend: HashtagTrend | TopicTrend
